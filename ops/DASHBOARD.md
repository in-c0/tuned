...
| --- | --- | --- |
| **L-104** | **A claim about the evidence, generated from its absence — and the instrument was my own.** The new EXP-013 reading printed *"the 9 live screen(s) carry **0** distinct top selection(s). Every screen chose the same candidate."* Read it twice: it reports finding **zero** identities, then describes **what the screens chose**. The identity was read as `record.find.key` — the label `agent-scout.mjs` **logs** — while it **serialises** `idempotencyKey`, so every screen came back anonymous, and the renderer's `keys.size <= 1` folded *none* into *one*. | The `<= 1` is the keeper: it was written for "all the same" and **looks** like it handles "none at all" for free, because both are "not many". They are opposite claims — **one** identity is about the evidence, **zero** is about the instrument. `<= 1`, `!x`, `length < 2`: the whole family describes the world using its own blindness as data. It was also the easiest possible place to get away with it: every other number this loop publishes is checked against something outside itself, and **a sentence about my own screening records is checked against nothing**. The true reading — **two** distinct selections, six and three — differs in a way no gate, test or reviewer would have caught, and I had already hand-read three logs and concluded the opposite. **The instrument corrected me; nothing would have corrected the instrument.** Worst of all it was *quotable*: a wrong claim that is boring gets checked, and this one was two-thirds true — the conclusion survived while the number under it was invented. **Rule:** when a reading describes its own subject, the no-evidence case gets its own branch and its own sentence, and that sentence says it **cannot be read**. A number sourced from an absence is not sourced, and absence is the one source that never fails to return a value. |
| **L-103** | **Four counter axes were subtracted from a bucket they were not drawn from, and one had been returning a negative number in production for a week.** `item_view_onsite` was written on **every** on-site find-page request regardless of the `_bot` split, while `item_view` holds the **non-bot bucket alone** — so the published rule `item_view - item_view_onsite` returned **-12, -4, -7, -47, -24** on 2026-09-16 -> 09-20. A crawler following a permalink off our own feed page decremented the human count. `follow_duplicate`, `desk_follow_duplicate` and `attention_star_owner` carried the identical defect on all-zero data, which is worse rather than better. | *The number was in front of the loop the whole time and nobody did the arithmetic* — [METRICS.md](METRICS.md) quotes the axis at **19** in a paragraph reporting its bucket at **7**, adjacent clauses, and `7 - 19` was never taken. **A reading rule that lives only in prose is never executed, so it is never observed to be false.** The suite had the same blind spot: every `item_view_onsite` test asked with a human UA — the one arrangement in which merging is invisible — and `test/activation.test.ts` **asserted the arithmetic that made the reading wrong**. Ask of a new axis not *is it a bucket* (all four correctly said no) but **does anything subtract me from a name I am not drawn from**. Prevention is the prose rule made executable; **the positive control deletes the merged-day exclusion and reddens the real-snapshot test**, printing the five negative days, which is what says the check is not vacuous. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-23 14:35 Sydney (2026-09-23 04:35 UTC), run 186. |
...
| **Freshness state** | **RESYNCHRONIZED for the header, §7 and §8, which were two runs behind at run 180; §1-§6 unchanged and not re-argued.** §1 is **ONE** and unchanged since run 137, per [L-07](LESSONS.md); this run adds none and **nothing in it needs the owner**. §4's funnel figures are unchanged: `applications` 0, `members` 1, `members_ever_active` 0, `followers` 0, gross cash AUD $0 — **no commercial metric moved this run and none is claimed.** `items_public` is **89** and did not move: the publisher's gate read **CURRENT**, so nothing was published, amended, retracted or restored. §6 still reads EXP-011 **CLOSED and graded**; **EXP-013 is untouched — its window closes 2026-09-25 for a reading due 2026-09-26, no threshold was graded early, and the daily schedule is still NOT armed.** **No counter was added, renamed, split or retired.** |
