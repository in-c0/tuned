...
| --- | --- | --- |
| **L-91** | **The file said which days it reported and never which of them had finished.** Every pre-registered reading here is defined over *complete* UTC days, and nothing in the repository could tell a complete day from a partial one: a snapshot taken at 23:05Z writes a row for today that is indistinguishable, at the point of use, from the same row taken after midnight — the fact lives one level away in `generated_at` and nothing compared them. `ops/METRICS.md` carries ~10 hand-written *"this day is partial"* notes, and [L-37](LESSONS.md#l-37) records run 57 dividing an unfinished day by elapsed hours and publishing the quotient as a cadence. **The mechanism built to remove the hazard was withdrawn by something nobody was watching:** `metrics-snapshot.yml`'s `15 0 * * *` schedule exists, in its own header's words, so the previous day is on disk *"complete, within minutes of ending"* — and its six most recent fires landed **4.48h–4.70h late, none inside fifteen minutes**. Two instruments in this repo already measure scheduler lateness; the one design depending on punctuality asked neither. | **A fact a repository holds and does not grade will be read as decoration, including by the run that wrote it down** — the fourth consecutive run in this family (L-88 the unread status code, L-89 the stylesheet not the element, L-90 the rule whose check read one page), and the first where the ungraded fact is *time*. Shipped: `scripts/metrics-window.mjs` answers *had every day in this window finished when this file was written?* and fails closed on an unreadable `generated_at`, an empty `daily`, a short history and an inverted window; the snapshot's commit message now carries `complete through <day>`, so `git log -- ops/metrics/` is a ledger. **Eight mutations, named tests red on each.** Stated plainly: **it did not rescue EXP-011** — run 166's regression clause had already moved that window out of the hazard's path, so the guard returned ADMISSIBLE on the first call. It is prospective. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-19 08:35 Sydney (2026-09-18 22:35 UTC), run 173. |
...
| **Repository commit at time of writing** | run 173 — `scripts/metrics-window.mjs` (+ its test), two first-party user-agent assertions in `test/metrics.test.ts`, one commit-message line in `.github/workflows/metrics-snapshot.yml`, and the ops record carrying EXP-011's graded reading. **`src/` is untouched, so the deployed Worker is byte-identical to `445f980`** — nothing deployed and no rollback path was required. **423 vitest** (421 → 423) · **ops suite 237/237** (215 → 237) · `npm audit --omit=dev` 0 vulnerabilities. **Twelve mutations, named tests red on each**, every source file restored byte-identical. |
...
| **Freshness state** | **RESYNCHRONIZED for the header, §6, §7 and §8; §1–§5 unchanged and not re-argued.** §1 is TWO and unchanged since run 143, per [L-07](LESSONS.md). §4's funnel figures are unchanged: `applications` 0, `members` 1, `members_ever_active` 0, `followers` 0, gross cash AUD $0 — **no commercial metric moved this run and none is claimed.** §6 now reads EXP-011 **CLOSED and graded** at R = 0.58%, Fork R-A. The **data commit row is unchanged by design** — no snapshot was taken this run and no counter was added, so [`metrics/latest.json`](metrics/latest.json) is still `b58c35a`, which is the very source the reading was taken from and whose admissibility was checked mechanically. |
