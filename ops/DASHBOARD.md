...
**Retired 2026-08-24 09:35:56 UTC (19:35 Sydney) on its own clock, unanswered.** The card below asked
for one directory submission; it required `@sportstech`'s newest public item to be under 72 hours old,
and that window closed at the instant above with nothing submitted.

| | |
| --- | --- |
| **Preserved** | **Your `A` (2026-08-20 15:04 UTC).** Granted, standing, and not re-asked when the candidate resumes. |
| **Paused, not dropped** | The `awesome-rss-feeds` submission. You did not say **C**, and an unanswered card is not a decision. |
| **What restores it** | One find worth publishing on its own merits. **Not scheduled.** Nothing was published to hold the window open — that is disqualified by [EXP-008](EXPERIMENTS.md)'s clauses, and this is the second window let go under that rule. |
| **What may not be concluded** | Anything about demand. No submission was made, so [EXP-009](EXPERIMENTS.md) Reading 2 has no `t0` and is **not graded**. |
| **If you want it anyway** | Open the issue whenever you like using the retired card below — but say so, because freshness must be re-read from production first and **does not hold right now**. No clock, no penalty. |

---

**Previously here, and retired unused — the submission card.** Kept intact because it is the card that
resumes when freshness returns. **Its A4 row has expired; do not act on it without a fresh read.**
