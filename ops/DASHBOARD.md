...
| **L-86** | **The change that broke the pull-request gate was merged without passing through it.** `check` runs on `pull_request` and on pushes to `master`, and from **2026-09-12** it could not be green on any pull request carrying a commit: one test asked the deploy watchdog whether a production serving `HEAD` reads fresh, on the premise its own comment stated — *"HEAD is what a checkout of master serves."* On a branch `HEAD` is not on master's first-parent line, so the CLI answered `unknown-serving` and exited 1, **correctly for the question it was asked**. The test landed in [`3e76d24`](https://github.com/in-c0/tuned/commit/3e76d24) **pushed straight to `master`** — across 79 `pull_request` runs of this workflow there is **none between 2026-08-27 and 2026-09-16**, so no pull request was open and nothing ever ran it on a branch. [Run 283](https://github.com/in-c0/tuned/actions/runs/35156879096), the only failing `pull_request` run in the workflow's history, was then diagnosed correctly and **merged past**. | **A gate is only a gate where it is enforced, and its own event distribution is a reading:** a check configured for two triggers that has lately passed on only one has either not been exercised on the other or cannot pass there, and both are findings. Shipped: the test's subject is master's **first-parent tip** rather than `HEAD`, resolved from **git directly and not from the function under test** — that shortcut is [L-79](LESSONS.md#l-79)/[L-84](LESSONS.md#l-84)'s shape. The CLI is unchanged; a production serving a commit off master's line is a real fault and still reads `unknown-serving`. A second test **builds the branch case in a throwaway repository** and asserts both directions, so the branch question is now asked **on `master` too** — the one place it could never be asked before, and the gap a direct push walked through. **Two mutations, two named tests red each.** Acceptance evidence is [PR #68](https://github.com/in-c0/tuned/pull/68)'s own [check run 286](https://github.com/in-c0/tuned/actions/runs/35180746550) — a `pull_request` event, green, the first since 2026-08-27. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-18 08:35 Sydney (2026-09-17 22:35 UTC) |
...
| **Repository commit at time of writing** | [`8433a13`](https://github.com/in-c0/tuned/commit/8433a1365a927b4486d98af32deb927d64373883) — run 170's coverage repair, **QA harness and one unit-test file only**: `qa/settle-requests.mjs` (+`trackHttpErrors`), `qa/settle-requests.d.mts`, `qa/public-surfaces.spec.mjs` (records **and grades** `firstPartyHttpErrors`, plus a route-level declared-asset test), `qa/pulse-instrument.spec.mjs` (records `http_errors`; console errors now carry `location().url`; **deliberately not graded**) and `test/settle-requests.test.ts` (+8 tests, 389 → 397; **three mutations each turning named tests red**). **`src/` is byte-identical**, so the Worker is byte-identical, the landing page is byte-identical, and EXP-011's window, thresholds and 2026-09-19 reading are unaffected. `check` [295](https://github.com/in-c0/tuned/actions/runs/35281357919) green on a `pull_request` event. Production evidence is two dispatches rather than a gate: [qa-browser 43](https://github.com/in-c0/tuned/actions/runs/35281317448) and [44](https://github.com/in-c0/tuned/actions/runs/35281433150), both clean against production serving `2740d05`. Earlier: run 169's RSS provenance line (`src/pages.ts`, one hunk inside `rssFeed`) and run 168's gate repair (`scripts/deploy-staleness.test.mjs`). |
| **Data commit** | [`a6a6476`](https://github.com/in-c0/tuned/commit/a6a6476) — [`metrics/latest.json`](metrics/latest.json), snapshot of `2026-09-17T04:54:05.899Z`; no snapshot has run since run 169. **No commercial metric moved this run and none is claimed.** Nothing was published, amended, retracted or restored. |
