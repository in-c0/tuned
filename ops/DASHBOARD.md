...
| --- | --- | --- |
| **L-98** | **A remedy queued behind an unanswered question is the same stoppage, one level up.** Run 179 found [L-97](LESSONS.md#l-97) — eight screens selected about nine candidates each and published none, because the record of what was waiting is a workflow artifact no run is obliged to open. It named the remedy in its own words, *make the backlog legible to the run that is supposed to attend the gate*, and **deferred it** on two grounds: rule 7 (control plane is not the product), and that it "only pays off if the gate stays attended — which is question 1", the reviewer's unruled EXP-013 re-specification. **That question was 20 days old when it was used as the reason to wait, and no reviewer directive has been posted since 2026-09-01.** | The fix for a gate nobody attends was filed behind a question nobody is answering, which is L-97's own shape recursed and presents identically: the feed stays quiet, every check stays green, and the run that deferred has a written reason. **A deferral is only a deferral if something is expected to arrive**; when nothing is, it is a decision not to do the work and should be argued as one. L-07 says escalate a blocker once and stop restating it — **stop restating is not stop acting around it.** Put a date on every deferral and name the event that would end it. **A second finding, from the mutation battery:** the instant walk started a day early behind a comment claiming it prevented a skip, and **no mutation could redden it** — a defensive line no test can pin is decoration read with a safeguard's authority. Removed. **Twelve mutations, named tests red on each**, including a positive control. |
| **L-97** | **A gate with nobody standing at it is an outage, not a safeguard.** Run 153 moved `agent-scout.yml`'s publication behind an explicit dispatch — *"by someone who has read the screening record"* — because EXP-013's quality threshold had failed. Sound. But between **2026-09-13 and 2026-09-20 the schedule fired eight times**, every run succeeded, and every run selected **about nine publishable candidates from about thirty-seven screened**. **Every one was discarded.** `@sportstech` went nine days without publishing while its publisher found something worth publishing daily. | **Moving an action behind an attended gate does not remove the action — it creates a queue**, and a queue whose depth and age nothing reports is indistinguishable from an empty one. The safeguard degrades silently into a stoppage, in the direction that looks calm: *a gate nobody attends and a pipeline with nothing in it produce the same observable.* The screening records are workflow artifacts nothing in the repository reads, and the operating card's read order reaches no artifact — so runs 176, 177 and 178 each found a real defect in the plumbing between a feed and a member and **none asked whether the feed had anything to put through it**. Ask of any gate: *what does a run that should act on this read, and is it in something a run is guaranteed to open?* The schedule was **not** armed — that boundary is a predecessor's pre-commitment and an unruled reviewer question. **Six mutations, named tests red on each**, including a positive control. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-21 14:35 Sydney (2026-09-21 04:35 UTC), run 180. |
...
| **Repository commit at time of writing** | run 180 — `scripts/scout-gate.mjs` (new), `scripts/scout-gate.test.mjs` (new), `scripts/operating-card.test.mjs` (one assertion: the card must name the reading, in its read order), `CLAUDE.md` (one step), and the ops record. **No worker source touched at all** — no route, no query, no CSS rule, no counter added, renamed, split or retired, so `464 vitest` is unchanged and the deployed Worker changes only by its build commit. **ops suite 265/265** (248 → 265, seventeen new) · `validate-nominations.mjs` **9 valid** · `npm audit --omit=dev` 0 vulnerabilities. **Twelve mutations, named tests red on each**, including a positive control — inferring the screen count from the publication's age in whole days is plausible, well-formed and wrong. Card and script restored byte-identical under `sha256sum -c`. |
| **Data commit** | [`142d7b8`](https://github.com/in-c0/tuned/commit/142d7b8) — [`metrics/latest.json`](metrics/latest.json), snapshot generated `2026-09-20T22:40:29.776Z`, complete through 2026-09-19. **No commercial metric moved this run and none is claimed:** `applications` 0, `members` 1, `members_ever_active` 0, `followers` 0, gross cash AUD $0 from *no billing exists*. `items_public` is **88** after run 179's item 282. **No counter was added this run**, and nothing was published, amended, retracted or restored. |
| **Freshness state** | **RESYNCHRONIZED for the header, §7 and §8; §1–§6 unchanged and not re-argued.** §1 is **ONE** and unchanged since run 137, per [L-07](LESSONS.md); this run adds none and **nothing in it needs the owner**. §4's funnel figures are unchanged: `applications` 0, `members` 1, `members_ever_active` 0, `followers` 0, gross cash AUD $0 — **no commercial metric moved this run and none is claimed.** §6 still reads EXP-011 **CLOSED and graded** at R = 0.58%, Fork R-A; this run registered no experiment and touched no threshold. **No counter was added, renamed, split or retired**, and nothing was published, amended, retracted or restored. |
