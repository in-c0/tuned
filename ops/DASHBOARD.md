...
| **L-85** | **The check measured the fault, printed it in its own artifact, and graded something else.** [L-84](LESSONS.md#l-84)'s new spec measured 16 public pages on production and returned **`brokenCount: 0`** — while reporting, in the same JSON and on the same page, `<span class="a">` on `/ava` **371.2px wide, ending at 418.2px on a 390px device**. Both were correct. The span cannot move either graded number: `.card.rollup` sets `overflow: hidden`, which is precisely the declaration that keeps `innerWidth` and `scrollWidth` honest **while destroying the content**. The element list had been collected on every page since the file's first version — as colour for a failure, never as a predicate — so run 166 read it, registered it, and shipped a **verified** deploy whose own verification artifact held a measured defect. Reproduced locally it was worse than the production reading said: **the track title in that row had been shrunk to 0px**, a symptom the offender list is structurally unable to report, because a box squeezed to zero is past no edge. | **A field a check collects and does not grade will be read as decoration, including by the run that wrote it** — and a green verdict is trusted in proportion to how much the check was seen to examine, so a check that looks at more than it grades converts its own thoroughness into false confidence. Either grade what you collect, or say in the file why a collected field is deliberately not a gate. Shipped: one CSS rule giving the artist's name a break opportunity so it **wraps rather than being truncated**, the same trade L-84 made for a source's name; the title returns and ellipsises honestly. `qa/mobile-fit.spec.mjs` now grades `overEdge` alongside the two page-level readings — **red on the unchanged build naming that exact span and measurement, green on the fixed one, and green on production at `b16441e9` with all 16 readings carrying `offenders: []`**. **Four mutations, four named tests red**, one of them asserting a rollup renders the span the rule selects at all: nothing in the suite had ever rendered one, and a rule whose selector matches nothing is [L-81](LESSONS.md#l-81)'s shape in CSS. |
| **L-84** | **The overflow check compared two numbers that the overflow moves together.** Production's landing page and demo feed did not fit a phone and had not for an unknown length of time: at 390px, **`/` laid out at 405px and `/ava` at 436px**. Chrome does not scroll such a page sideways — it **zooms the whole document out**, so every word on the only page a visitor can apply from was rendered smaller than designed. Two specs already asserted overflow at exactly this viewport, both as `scrollWidth <= innerWidth + 1`. That is a real check, and it is **structurally blind** here: the failure moves *both* numbers, so they stay equal and the assertion is **greenest at the moment the page is worst**. | **A check that compares two numbers is only as good as their independence** — when a failure can move the reference as well as the reading, the discriminator has to come from outside the thing under test. Shipped: four CSS rules in the **shared** stylesheet (the landing page renders its demo through the same `card()`, so a page-scoped block would have left it broken), and `qa/mobile-fit.spec.mjs`, which compares the layout viewport against the width the project **asked for**, keeps the sideways-scroll assertion since neither covers the class alone, and reads its page list from the live sitemap so a surface published later is covered the day it is published. `overflow-wrap`, not `text-overflow`: a source reading `blog.engineering.longsub…` is a worse answer than one taking two lines. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-17 14:35 Sydney (2026-09-17 04:35 UTC) |
