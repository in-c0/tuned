...
| **L-87** | **The checks that read a surface are not the checks a gate runs.** Run 169 changed the shape of an RSS item's `<description>` and broke `qa/exp008-provenance.spec.mjs`, which asserts a nomination's why line against that exact field with an exact string equality. `qa-browser` is **dispatch-only by design** — recurring headless traffic through production's own funnel counters would corrupt them — so six green gates, a green `pull_request` check and a green `verify production` said **nothing at all** about it. It was found by grepping `qa/` for the surface **after** the merge. | **"The tests pass" answers a question about the tests a gate runs, and a repository's checks are a larger set than that.** A dispatch-only check is not weaker coverage — it is coverage with **no trigger**. Before changing a surface, grep every check that *reads* it, not every check a gate *runs*. Shipped: the repair **moves layer rather than relaxing** — what EXP-008 grades is that the *whole* why line reaches a subscriber untruncated, so the comparison stays exact and re-aims at the paragraph now carrying it; `toContain` would have graded the same words and stopped grading "whole". |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-18 14:35 Sydney (2026-09-18 04:35 UTC) |
...
| **Repository commit at time of writing** | [`f394af2`](https://github.com/in-c0/tuned/commit/f394af264233b7681c17b1ac8bead05c6a2fa210) — run 171's follow affordance on the find page. `src/pages.ts` (`itemPage` gains a follow block and dialog; `FIND_JS` wires it; new page-scoped `FIND_CSS`), `src/index.ts` (`find_follow_open`/`find_follow_rss` on the pulse allowlist, `follow_submit_find`/`follow_invalid_find` axis on `POST /<handle>/follow`), `src/metrics.ts` (the published note), plus `test/permalink.test.ts`, `test/follow.test.ts` and the `qa/pulse-instrument.spec.mjs` allowlist mirror — **408 tests, 397 → 408**, and **six mutations each turning named tests red**. **`src/pages.ts` is untouched above line 942**, so the landing page and the feed page are byte-identical and EXP-011's window, thresholds and 2026-09-19 reading are unaffected. `check` [298](https://github.com/in-c0/tuned/actions/runs/35306568496) green on a `pull_request` event; `verify production` [280](https://github.com/in-c0/tuned/actions/runs/35306666842) green at this commit; [qa-browser 45](https://github.com/in-c0/tuned/actions/runs/35306755727) — `mobile-fit` against production serving `f394af2` — **16 pages, `brokenCount: 0`, eight of them find pages**. Earlier: run 170's HTTP-error collector and run 169's RSS provenance line. |
