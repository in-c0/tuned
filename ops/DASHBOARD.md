...
| **Repository commit at time of writing** | run 177 — `src/pages.ts` (the desk offer in both follow dialogs), `src/index.ts` (`feedViewer`, the private-cache header, the surface axis), `src/metrics.ts` (snapshot note), `.github/workflows/verify-production.yml` (one new step), `test/activation.test.ts`, the new `test/desk-offer-render.test.ts`, `test/route-inventory.test.ts`, `test/env.d.ts`, and the ops record. **457 vitest** (437 → 457, twenty new) · **ops suite 244/244** (unchanged) · `npm audit --omit=dev` 0 vulnerabilities. **Eleven mutations, named tests red on each**, every file restored byte-identical under `sha256sum -c`. |
