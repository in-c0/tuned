...
>
> **Everything else is done.** The feed is fresh again — **0.0h**, read from production at 09:37 UTC
> today, because `@sportstech` published a second find this run (**item 246**). Nothing at the venue
> mentioned Tuned as of **yesterday evening**, neither its issues nor its README, so this would not be
> a duplicate — that check is a day old and is not being re-claimed as fresh.
>
> ~~**Two alternatives, both fine.** … the executor could submit that unaided if you say so …~~
> **Struck run 65 — this sentence was withdrawn as false at run 62 and survived here for three days.**
> The executor holds **no instrument that can submit a form to anyone**: egress is 403 to every host,
> `docs.google.com` included, and its only third-party vantage is a GET-only reader. The form option
> would also give **no receipt and no link**, so a silent outcome could never be told apart from a
> submission that never arrived. **The remaining alternative is C — drop the candidate**, which still
> costs nothing. (Reviving the form is **option B** in [§1](#1-owner-action-required): a new capability,
> not a permission, and not recommended.)
>
> **The clock, stated plainly and not as pressure:** the freshness condition lapses **Monday
> 2026-08-24 at 19:35 Sydney**. **The previous window already lapsed unused, at 14:15 Sydney today**,
> and nothing was published to save it — the executor will not publish to hold a window open, and did
> not. Today's find reopened it because it cleared the feed's remit on its own. After Monday the
> submission waits for the next such find, which is not scheduled. **No penalty, nothing breaks.**
>
