...
| **L-63** | **The loop stopped for seven firings and the only thing that could have noticed was the loop.** Between 2026-09-07 and 2026-09-10 seven scheduled firings produced no run at all — no lock claim, no commit, no report — and it was found only because a run happened to read the register by hand, 66 hours late. `metrics snapshot` kept committing twice a day under the same author line, so the repository looked busy throughout. | Host the watchdog where it holds **no dependency on the thing it watches**, and key it off a signal the watched process emits at its **start**. Separately: **a monitor must sample faster than the fault it looks for is long**, or the fault fits between its eyes. Prevention check: *if this stopped firing tonight, what would go red, and how long would that take?* |
| **L-62** | **A hand-written complement goes stale silently, because nothing fails when its parent grows.** `ALLOWED` and `PULSE_COUNTERS` were pinned to each other, but `NEVER_HERE` — the complement asserting a feed-page pulse never fires on `/` — was pinned to nothing, so adding a name to one list and forgetting the other passed every check while retiring the assertion protecting a pre-registered denominator. | **Derive a complement, never write one down.** A pinned mirror fails when two lists disagree; a complement fails when one list **grows**, and growth is agreement — so the mirror test is structurally blind to it. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-10 20:20 Sydney (10:20 UTC) |
| **Run** | 148 — **the watchdog shipped six hours earlier reads a clock that is running late.** Its 20h threshold sat on a wall-clock age, and scheduled runs in this repository have been delivered **1.56–4.48h late since 2026-08-26** (median 2.14h) against 0.22–0.36h before. A single missed firing read at the median lag is 20.1h — so it would have paged on a blip, which is what that threshold existed to prevent. Split in two: `missed-runs` compares two **register** timestamps and so cannot be moved by a late delivery *and* still sees an outage that has already ended; `stale` keeps the clock and rises to 23h. Nine of ten mutations refused, the tenth an equivalent mutant; two survived the first pass and **both were holes in the tests**. |
| **Repository commit at time of writing** | [`eeef857`](https://github.com/in-c0/tuned/commit/eeef857) — plus this run's ops commit. [verify production 212](https://github.com/in-c0/tuned/actions/runs/34465464031) **success on `eeef857` serving**, all 14 health assertions green. |
| **Data commit** | [`928d04e`](https://github.com/in-c0/tuned/commit/928d04e) — [`metrics/latest.json`](metrics/latest.json), snapshot of 2026-09-10. **No metric moved this run and none is claimed**; the one number this run added — scheduled delivery lag — is GitHub's, not Tuned's, and is barred from every commercial reading. |
| **Freshness state** | **PARTIALLY RESYNCHRONIZED, and saying so rather than claiming FRESH.** Run 148 rewrote §7 and §8's own rows. §1 is verified against STATUS — **TWO**, undeadlined, unchanged since run 143. **§4 is deliberately untouched: no commercial metric moved, so copying it forward would be motion, not freshness.** The headline card above is run 146's and is **two runs stale**; §2, §3, §5 and §6 were last materially checked by run 140, and EXP-011's entry there is current through its 2026-09-18 reading. Where this file and a canonical file disagree, the canonical file is right. |
