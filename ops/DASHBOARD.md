...
| --- | --- | --- |
| **L-90** | **The honesty rule had a scope, and every surface that converts was outside it.** This loop has a written rule against a page claiming freshness it does not have ([L-18](LESSONS.md#l-18)), and a check enforcing it that has been green on every run since 2026-08-13. **That check reads the front page and nothing else.** Meanwhile run 150 put a Follow button on every feed page and run 171 put one on all eighty-seven find pages, each promising finds "as they are published" — and a reading of the live site this run found **four of the five feeds have published nothing since 30 July**, with **sixty-eight of those eighty-seven pages belonging to one of them**. The rule's own check could not see either surface, so it kept passing while two runs shipped the claim it exists to catch. | **When a rule acquires a check, the check's scope quietly becomes the rule's scope** — and nobody notices the day a new page falls outside it. Adding a page that makes the kind of claim an existing check grades is the moment to ask what that check actually reads, not the moment to trust that it is green. Shipped: the pages **state the age** — *"the last was 49 days ago"* — rather than the check being widened, because a date rendered from the row cannot go stale and needs nothing to keep it true. Unconditional, with **no threshold**: a cut-off picked here would be a number fitted to the five feeds I can see. **A second mistake of my own, same run:** I put the age in bold, and bold in that block is styled as its own line, so one sentence became three with the full stop stranded on a row by itself — and the layout check read **clean**, because nothing overflowed. A check that only measures overflow cannot see a layout that is merely wrong. |
| **L-89** | **The assertion found the stylesheet, not the element, and passed however the page was ordered.** Run 171 gave the find page a follow block and wrote a test for the one property that keeps it on the right side of the doctrine boundary — *the outbound link to the source must come before the follow ask*. It was written as `indexOf("open-cta") < indexOf('id="follow-btn"')`, it passed, and it was **vacuous**: `layout()` inlines the whole stylesheet into `<head>` and `CSS` contains an `.open-cta` rule, so `indexOf` found the **rule**, in the head, before the body whatever the body contains. The mutation that moves the follow block above the CTA left it **green** — with every gate green, the suite green, and the claim *"graded by document order rather than left to inspection"* already written into the commit message. | **A substring is not a locator.** An assertion about *where* something sits in a document must match markup that exists exactly once and only in the place being asserted about — here `<a class="btn primary open-cta"`, not the class name, which the same document inlines in its stylesheet. Generally: when a check searches serialized output for a *name*, ask which other parts of that output legitimately contain it — CSS selectors, `<link>` hrefs, `<script>` bodies and meta tags all carry class names, route names and counter names, in the same string. Shipped: rewritten against the anchor's own markup and re-mutated **red**. The neighbouring precaution taken for the same reason: `find_follow_rss` **contains** `follow_rss`, so the assertion forbidding feed-page counters on a find page uses a lookbehind rather than `not.toContain`, which would have passed on a page emitting exactly the names it forbids. **Second consecutive run in which the mutation pass, not the test run, found the defect** — and the first in which it found one in a test written the same hour. |
| **L-88** | **A 404 is not a request failure, so nothing in the repository graded one.** Run 169's landing bracket recorded one console error — *"Failed to load resource: the server responded with a status of 404 ()"* — and could not say **which** resource, because the spec stored `m.text()` and not `m.location().url`: [L-85](LESSONS.md#l-85) inside this loop's own instrument, one run after L-85 was written. Going after the missing field found the larger fault: **there was no check that could have caught the 404 at all.** Every browser spec in `qa/` grades `requestfailed`, and a 404 is not a request failure — the server answered. `qa/settle-requests.mjs` **states that fact in its own header** and uses it only to argue about which aborts to exempt. So a subresource declared on our own origin could 404 on **every page load, on every public surface**, with `firstPartyRequestFailures` empty and all six gates green. | **An empty failure list is only as wide as the event it listens to** — `requestfailed` answers *did the transport break*, not *did the server refuse*, and a check that grades one while being read as grading the other converts an unexamined surface into a green one. When a spec asserts a list is empty, name the event that fills it and ask what it **cannot** contain. Shipped: `trackHttpErrors` grades first-party responses with status ≥ 400 in `public-surfaces.spec.mjs`, naming url, status and resourceType; `pulse-instrument.spec.mjs` records the same and **is deliberately not graded**, being EXP-011's apparatus eight days before its reading. Plus the half no browser instrument can reach: **`og:image` is never requested by the page that declares it**, so a route-level test fetches the declared same-origin assets on purpose and requires 200 **with an `image/*` content-type** — a 200 serving the HTML 404 page passes a status check and still unfurls as nothing. **Three mutations, named tests red on each.** Production came back **clean** — the 404 does not reproduce and, because the artifact never kept the URL, **can never now be attributed**. |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-18 20:35 Sydney (2026-09-18 10:35 UTC) |
...
| **Repository commit at time of writing** | [`6cab57b`](https://github.com/in-c0/tuned/commit/6cab57b78b5559cd3c113fb5508da97f4734b424) — run 172's cadence disclosure on both follow surfaces. `src/pages.ts` only (`lastPublished()`, `lastPublishedClause()`, the find page's follow block, and both follow dialogs) plus the new `test/follow-cadence.test.ts` — **421 tests, 408 → 421**, and **eight mutations each turning named tests red**. **`landingPage` is byte-identical, asserted against `master` rather than claimed**, and **no CSS rule was added**, so EXP-011's window, thresholds and 2026-09-19 reading are unaffected and `FEED_CSS`/`FIND_CSS` are unchanged. `check` [105565006610](https://github.com/in-c0/tuned/actions/runs/35334146869) green on a `pull_request` event, with Workers Builds and GitGuardian green; the reading it rests on is [qa-browser 46](https://github.com/in-c0/tuned/actions/runs/35333163076), `EXP005_SUMMARY` at `2026-09-18T10:08:54Z` against `b58c35a`. Earlier: run 171's find-page follow affordance and run 170's HTTP-error collector. |
| **Data commit** | [`4414ef1`](https://github.com/in-c0/tuned/commit/4414ef1) — [`metrics/latest.json`](metrics/latest.json), snapshot of `2026-09-17T23:02:45.112Z`. **No commercial metric moved this run and none is claimed.** Four counters were added and all four start at zero on this deploy: `find_follow_open`, `find_follow_rss`, `follow_submit_find`, `follow_invalid_find`. Nothing was published, amended, retracted or restored. |
| **Freshness state** | **RESYNCHRONIZED for the header, §7 and §8, and not for §1–§6.** §1 is TWO and unchanged since run 143 and is not re-argued, per [L-07](LESSONS.md). §4's funnel figures are unchanged: `applications` 0, `members` 1, `members_ever_active` 0, `followers` 0, gross cash AUD $0 — **no commercial metric moved this run and none is claimed.** §6 still reads EXP-011 pending its **2026-09-19** reading, which this run did not touch. The **data commit row is one run stale by design** — no snapshot was taken this run and no counter was added, so [`metrics/latest.json`](metrics/latest.json) is unchanged since `b58c35a`. |
