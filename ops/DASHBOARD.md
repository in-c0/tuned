...
| **L-106** | **A threshold I had filed as "needs production, which I cannot reach" was failable from a file in git all along — by 2.8x.** EXP-013's threshold 4 is *"the newest public item is ≤ 72h old on every reading in the window."* Confirming freshness **held** does need the live site. Showing it **lapsed** needs nothing but arithmetic on `publishedAt` timestamps already committed to `qa/nominations/`: the longest gap in the window is **203.8h**. | ***"I cannot reach the source that grades this"* is not *"this cannot be graded."*** Before deferring a threshold to a source you lack, ask which **direction** that source is needed for — a bar is often **failable from the cheap source and passable only from the expensive one**, because a lower bound establishes a failure and can never establish a pass. Deferring the whole threshold throws away the half you could have read, and it is easiest to do when the expensive source is *genuinely* unreachable, because the unreachability feels like the whole answer. **Second half:** when a fork carries an action, check whether that action makes one of the experiment's own thresholds unachievable. Fork B switched the publisher off on the window's first day, so threshold 4 could only fail thereafter — **a threshold whose outcome your own fork determined measures your design, not your subject.** |
...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-24 20:35 Sydney (2026-09-24 10:35 UTC), run 190. |
