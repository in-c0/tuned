...
| | |
| --- | --- |
| **Last materially updated** | 2026-09-11 08:20 Sydney (2026-09-10 22:20 UTC) |
| **Run** | 149 — **the advisory run 148 waved off in one clause was the one that could write into two experiments' numerators.** `hono` 4.12.34 → 4.13.7. The old query parser read parameters sitting after a URL fragment, so `/sportstech#x?src=ooh-directory` minted an `arrival:ooh-directory` from a URL with no query string, and `/sportstech?src=ooh-directory#x` destroyed a real one. Those two names are EXP-009's and EXP-012's numerators — **wrong in both directions**, at the two venues in §1. Three regression cases pinned **by mutation**: reinstalling 4.12.34 fails all three, passes the other 28. `npm audit --omit=dev` **1 moderate → 0**. Separately, EXP-011's dated mid-window bracket came due and is **green** (`page_errors: []`, no hoist), and METRICS' open question on cron delivery is closed: the hourly watchdog is **rate-reduced to ~1 per 4h**, not phase-shifted. |
| **Repository commit at time of writing** | [`e9e2a00`](https://github.com/in-c0/tuned/commit/e9e2a00) — plus this run's ops commit. [verify production 214](https://github.com/in-c0/tuned/actions/runs/34536201483) **success on `e9e2a00` serving**, every health assertion green including the EXP-011 render beacon. [qa-browser 34536404000](https://github.com/in-c0/tuned/actions/runs/34536404000) confirms the landing emitter unchanged on the new build. |
| **Data commit** | [`928d04e`](https://github.com/in-c0/tuned/commit/928d04e) — [`metrics/latest.json`](metrics/latest.json), snapshot of 2026-09-10. **No commercial metric moved this run and none is claimed.** The one number resolved — scheduled cron *delivery rate*, ~1 per 4h against 24 requested — is GitHub's, not Tuned's, and is barred from every commercial reading. |
| **Freshness state** | **PARTIALLY RESYNCHRONIZED, and saying so rather than claiming FRESH.** Run 149 rewrote §7 and §8's own rows. §1 is verified against STATUS — **TWO**, undeadlined, unchanged since run 143. **§4 is deliberately untouched: no commercial metric moved, so copying it forward would be motion, not freshness.** The headline card above is run 146's and is **three runs stale**; §2, §3, §5 and §6 were last materially checked by run 140, and EXP-011's entry there is current through its 2026-09-18 reading. Where this file and a canonical file disagree, the canonical file is right. |
