...
> # **The single number this whole bet is waiting for was the one number we could not have checked. It is fixed, before it ever moves.**
>
...
>
> **What we found.** When someone applies to Tuned, the site records it. It records **nothing about
> where the application came from** — and it was the only thing on the whole site like that. Every
> other count we keep says whether it came from a real browser on our own page or from something
> automated. The application form did not, and the code for it is public, so anyone with a script
> could have written an entry that arrived on our dashboard looking exactly like a real person.
>
> **Why that matters more than it sounds.** Applications have read **0** every single day of this
> bet. The first day it reads **1** is the day everything changes direction — and that is precisely
> the number nobody would think to double-check, because it is the news we have been waiting sixty
> days for. **Good news does not get questioned.** It survived 140 runs for the same reason: it was
> never *wrong*, because it had only ever counted zero, and a counter that has only ever said zero
> looks exactly like a correct one.
>
> **What we did.** Applications now record two things about their origin. The weaker one is whether
> the sender declared itself automated — easily faked, and there for consistency. The stronger one is
> whether the request actually came from our own page in a browser, which a script cannot fake and
> almost never sends. That second marker is the one that would catch the realistic case.
>
> **What we deliberately did not do: turn anyone away.** We could have made the site *reject*
> applications that do not look like they came from our page. We did not, and will not. Applications
> are at zero, so wrongly turning away one real person — because their browser did something we did
> not expect — costs more than every mislabelled entry combined. **The site now labels applications;
> it never refuses them.** There is a test that will fail if any future change makes it refuse one.
>
...
>
> **One thing considered and turned down, on purpose.** That 18 September reading spends 13 of the 30
> days we have left, and it freezes the landing page — the one thing we fully control — for all of
> them. There is a legitimate statistical way to end it early, and tonight was the last honest moment
> to set one up. **We chose not to.** We had written down, in advance, that we would read the result
> once, on one named day, precisely so that we could not shorten it later when we got impatient. Being
> impatient now is not a good enough reason to break a rule we wrote for this exact moment.
>
