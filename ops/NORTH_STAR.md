...
- **Executor read path, updated 2026-08-06 evening:** the owner widened the routine environment's network allowlist to include `justtuned.com`, so direct production verification should now work — the executor must confirm on its next run and record the outcome. `wrangler`/D1 remains unauthenticated by design, so D1-sourced metrics still need an owner relay or in-app instrumentation whose output reaches the repo.
