...
- **2026-09-06 (run 143)** — **A5 closed at `ooh.directory`, and with it the last distribution work
  anywhere on this board that this executor could perform.** `ooh-directory` added to `ARRIVAL_TAGS`,
  so `arrival:ooh-directory` writes on `GET /:handle` — the route the venue's own form field selects
  (*"The URL of the blog's front page (not its feed)"*). Five tests pin it, including one asserting
  the tag writes **nothing** on `GET /`: that route reads no `?src=` at all, and pinning it stops a
  later run instrumenting the marketing page and silently changing what EXP-012's numerator counts.
  [EXP-012](EXPERIMENTS.md) pre-registers the window, `D ≥ 3 and V ≥ 8`, six forks (with
  **never-listed** named as the *expected* outcome, per the venue's own FAQ) and **two** controls —
  `arrival:qa`, whose measured null on this route is **0 non-zero days across 21**, and the tag's own
  published-but-unsubmitted interval before `t0`. Registered before the counter had ever been
  written, because counters do not backfill and a suggestion is spent once.
  [`SUBMISSION-ooh-directory.md`](SUBMISSION-ooh-directory.md) makes the act a paste. **Nothing was
  submitted and no venue was contacted.** Run 137 named this *"the next executor-actionable step at
  this venue"* and runs 138–142 each declined it for instrument work — [L-59](LESSONS.md).
  **Consequence for the whole register: every remaining step at every candidate is now A0 or A2, and
  both are the owner's.** `awesome-rss-feeds` has their `A` and needs two minutes; `ooh.directory`
  needs one authorship answer first, put to them verbatim in the packet, and it has never been asked.
